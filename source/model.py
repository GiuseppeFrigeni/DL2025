...
class NNConvNet(torch.nn.Module):
    def __init__(self, node_in_channels, edge_feature_dim, hidden_channels, out_channels, num_layers=2, dropout=0.5):
        ...
        edge_nn1 = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_channels * hidden_channels, hidden_channels * node_in_channels) # Or some other size
        )
        ...
        for _ in range(num_layers - 1):
            ...
            torch.nn.Linear(edge_feature_dim, hidden_channels * hidden_channels),
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_channels * hidden_channels, hidden_channels * current_dim)
            )
